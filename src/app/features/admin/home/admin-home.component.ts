  ...
  template: `
    <div class="mx-auto max-w-3xl py-12 text-center">
      ...
      <div class="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
        @if (auth.hasPageAccess('admin_usuarios.activos')) {
        ...
        @if (auth.hasPageAccess('admin_usuarios.pendientes')) {
        ...
        @if (auth.hasPageAccess('admin_usuarios.seguridad')) {
          <a routerLink="/admin/seguridad"
            ...
            <p class="mt-1 text-[13px] text-[var(--color-storm-cloud)]">Roles, módulos y páginas del sistema.</p>
          </a>
        }
      </div>
    </div>
