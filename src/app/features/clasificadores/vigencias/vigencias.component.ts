...
export class VigenciasComponent implements OnInit {
  ...
  form: VigenciaRequest = {
    ejercicio: new Date().getFullYear(),
    activoEjecucion: false,
  };
  ...
  openEditModal(vigencia: Vigencia) {
    this.isEditing.set(true);
    this.editingId.set(vigencia.idVigencia);
    this.form = {
      ejercicio: vigencia.ejercicio,
      activoEjecucion: vigencia.activoEjecucion,
    };
    this.isModalOpen.set(true);
  }
  ...
  save() {
    ...
    const obs = this.isEditing() && this.editingId() != null
      ? this.vigenciaService.update(this.editingId()!, this.form)
      : this.vigenciaService.create(this.form);
    ...
    obs.subscribe({
      next: (res) => {
        ...
        if (res.success) {
          this.closeModal();
          this.showSuccess(this.isEditing() ? 'Vigencia actualizada correctamente.' : 'Vigencia creada correctamente.');
          this.loadVigencias();
        }
      },
      error: () => {
        this.isSaving.set(false);
        this.errorMessage.set('Error al guardar la vigencia.');
      },
    });
  }
  ...
  confirmDelete(vigencia: Vigencia) {
    ...
    this.vigenciaService.delete(vigencia.idVigencia).subscribe({
      next: (res) => {
        if (res.success) {
          this.showSuccess('Vigencia eliminada correctamente.');
          this.loadVigencias();
        }
      },
      error: () => {
        this.errorMessage.set('Error al eliminar la vigencia.');
      },
    });
  }
  ...
  setActiva(vigencia: Vigencia) {
    this.vigenciaService.setActivaEjecucion(vigencia.idVigencia).subscribe({
      next: (res) => {
        if (res.success) {
          this.showSuccess(`Vigencia ${vigencia.ejercicio} marcada como activa.`);
          this.loadVigencias();
        }
      },
      error: () => {
        this.errorMessage.set('Error al activar la vigencia.');
      },
    });
  }
  ...
  vigenciaOrdenada = computed(() =>
    this.vigencias().sort((a, b) => b.ejercicio - a.ejercicio)
  );
}
