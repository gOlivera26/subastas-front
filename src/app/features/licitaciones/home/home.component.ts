...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-3xl py-12 text-center">
      ...
      <div class="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-4xl mx-auto">
        @if (auth.hasPageAccess('licitaciones.subasta')) {
        ...
        @if (auth.hasPageAccess('licitaciones.nota-pedido')) {
        ...
        @if (auth.hasPageAccess('licitaciones.informes')) {
          <a routerLink="/licitaciones/informes" class="group rounded-2xl border border-[var(--color-charcoal-grey)] bg-[var(--color-graphite)]/50 p-6 text-left transition-all hover:border-[var(--color-cyan-spark)]/30 hover:bg-[var(--color-deep-slate)]">
            <div class="mb-3 inline-flex size-10 items-center justify-center rounded-xl bg-[var(--color-cyan-spark)]/10"><lucide-icon name="bar-chart" [size]="20" class="text-[var(--color-cyan-spark)]"></lucide-icon></div>
            <h3 class="text-[16px] font-[590] text-[var(--color-porcelain)] group-hover:text-[var(--color-cyan-spark)]">Informes</h3>
            <p class="mt-1 text-[13px] text-[var(--color-storm-cloud)]">Reportes operativos, auditoría y resultado de subastas.</p>
          </a>
        }
        @if (auth.hasPageAccess('licitaciones.tablero')) {
          <a routerLink="/licitaciones/tablero" class="group rounded-2xl border border-[var(--color-charcoal-grey)] bg-[var(--color-graphite)]/50 p-6 text-left transition-all hover:border-[var(--color-neon-lime)]/30 hover:bg-[var(--color-deep-slate)]">
            <div class="mb-3 inline-flex size-10 items-center justify-center rounded-xl bg-[var(--color-neon-lime)]/10"><lucide-icon name="layout-dashboard" [size]="20" class="text-[var(--color-neon-lime)]"></lucide-icon></div>
            <h3 class="text-[16px] font-[590] text-[var(--color-porcelain)] group-hover:text-[var(--color-neon-lime)]">Tablero</h3>
            <p class="mt-1 text-[13px] text-[var(--color-storm-cloud)]">Vista ejecutiva y seguimiento de indicadores del módulo.</p>
          </a>
        }
      </div>
    </div>
