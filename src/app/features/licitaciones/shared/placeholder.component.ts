  ...
  template: `
    <div class="flex flex-col items-center justify-center py-20 text-center">
      <div class="mb-6 inline-flex size-20 items-center justify-center rounded-2xl border border-[var(--color-charcoal-grey)] bg-[var(--color-deep-slate)]">
        <lucide-icon [name]="icon" [size]="32" class="text-[var(--color-cyan-spark)]"></lucide-icon>
      </div>
      <h2 class="mb-2 text-[24px] font-[590] text-[var(--color-porcelain)]">{{ title }}</h2>
...
export class PlaceholderComponent {
  @Input() title = 'Módulo';
  @Input() description = 'Este módulo está en desarrollo.';
