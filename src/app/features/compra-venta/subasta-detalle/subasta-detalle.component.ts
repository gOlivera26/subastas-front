  ...
  export class SubastaDetalleComponent implements OnInit, OnDestroy {
    ...
    // CORREGIDO: Inicialización explícita de tipos como string para acoplarlos al select del HTML
    garantiaForm = {
      idTipoDocumento: '0', 
      idMoneda: '0', 
      companiaAseguradora: '', 
    ...
    this.chartOptions = {
      ...
      // CUSTOMIZACIÓN MÁSER: Agregamos el path del avión de OWEN en cada punto de oferta
      markers: {
        size: 6,
        strokeColors: '#0B0E14',
        strokeWidth: 2,
    ...
    // Effect: Procesar ofertas e inyectar puntos reactivos en tiempo real al Trading Chart
    effect(() => {
      ...
      // Ordenar cronológicamente para reconstruir la línea de tendencia patria
      ...
      pujasOrdenadas.forEach(puja => {
        ...
        if (idItem && bestPerItem[idItem] !== undefined) {
          ...
          // CORREGIDO: Evaluamos el cambio de precio usando la variable auxiliar ANTES de mutar el acumulador
          if ((!this.isDirecta && puja.monto < precioAnterior) || (this.isDirecta && puja.monto > precioAnterior)) {
            ...
            bestPerItem[idItem] = puja.monto; // Ahora sí impactamos el nuevo piso/techo
            dataPoints.push([new Date(puja.fecha).getTime(), totalActual]);
          }
        }
      });

      // Añadir punto de tiempo flotante actual para mantener viva la línea de tendencia en curso
      if (dataPoints.length > 0 && this.isLive) {
        dataPoints.push([this.timeService.now(), totalActual]);
      }

      // Actualizar inputs del formulario de forma inteligente
      ...
      // Renderizar la serie en la gráfica de ApexCharts
  ...
  loadMonedas() {
    ...
    guardarGarantia() {
      ...
      // Construcción limpia y emparejada con GarantiaRequestDto de C#
      ...
      if (tipoDoc === 1) { 
        ...
        // Enviamos nulos explícitos para evitar ruidos en el bindeo de ASP.NET
        ...
        // Enviamos vacíos explícitos para los campos de póliza
        formData.append('CompaniaAseguradora', '');
        formData.append('NroPoliza', '');
        formData.append('MontoCaucion', '0');
      }

      // CORREGIDO: Consumimos el servicio real inyectado que pasa por el Interceptor de JWT
